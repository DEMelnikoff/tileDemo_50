...
var streakGame = (function() {
    ...
    let alpha_url = jsPsych.data.getURLVariable('alpha')
    let nTrials_url = jsPsych.data.getURLVariable('nTrials')

    if (alpha_url == 5 | alpha_url == 9) {
        settings.pM = alpha_url;
    };

    if (nTrials_url >= 10) {
        settings.nTrials = nTrials_url;
    };

    if (settings.pM == 5) {
        settings.colorOrder = 1;
    } else {
        settings.colorOrder = 0;
    };
    ...
    function MakeFeedback(round, span, game) {
        ...
        this.stimulus = function(){ 
            ...
            if (round == 'R1' && settings.streakOrder == 1 || round == 'R2' && settings.streakOrder == 0) {
                if (img == "failure") {
                    feedbackText = `<div style='font-size: 50px'>Failure!</div>`
                } else {
